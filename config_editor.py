...
class ConfigEditorApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(self.root, text="WPM Limit (words/min):").grid(row=2, column=0, padx=10, pady=10)
        self.wpm_limit_var = tk.StringVar(value=self.config['wpm_limit'])
        self.wpm_limit_entry = tk.Entry(self.root, textvariable=self.wpm_limit_var)
        self.wpm_limit_entry.grid(row=2, column=1, padx=10, pady=10)
        
        tk.Label(self.root, text="WPM Tracking Duration (sec):").grid(row=3, column=0, padx=10, pady=10)
        self.wpm_td_var = tk.StringVar(value=self.config['wpm_td'])
        self.wpm_td_entry = tk.Entry(self.root, textvariable=self.wpm_td_var)
        self.wpm_td_entry.grid(row=3, column=1, padx=10, pady=10)
        ...
        self.folder_button.grid(row=4, column=0, columnspan=2, pady=10)
    ...
    def select_folder(self):
        ...
        if folder_path:
            ...
            # Check if "math_problems" folder already exists and remove it
            if os.path.exists(destination_folder):
                try:
                    shutil.rmtree(destination_folder)
                except Exception as e:
                    messagebox.showerror("Error", f"Failed to remove existing folder: {str(e)}")
                    return
    ...
    def save_config(self):
        ...
        new_wpm_limit = self.wpm_limit_var.get()
        new_wpm_td = self.wpm_td_var.get()

        # Validate inputs (ensure they are numeric)
        if not new_min_words.isdigit() or not new_time_limit.isdigit() or not new_wpm_limit.isdigit() or not new_wpm_td.isdigit():
        ...
        with open(self.config_file, 'w', newline='') as file:
            ...
            writer.writerow(["wpm_limit", new_wpm_limit])
            writer.writerow(["wpm_td", new_wpm_td])
